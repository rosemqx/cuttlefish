...
map_value_sub(Schema, Config) ->
    io:format("Right Hand Side Substitutions"),
...
apply_mappings({Translations, Mappings, _Validators}, Conf) ->
    ...
    {DirectMappings, {TranslationsToMaybeDrop, TranslationsToKeep}} = lists:foldr(
        ...
        Mappings),
    io:format("Applied 1:1 Mappings"),
...
apply_translations({Translations, _, _} = Schema, Conf, DirectMappings, TranslationsToDrop) ->
    ...
    case Errorlist of
        [] ->
            io:format("Applied Translations"),
            Proplist;
        Es ->
            ...
            case lists:member(Mapping, TranslationsToDrop) of
                false ->
                    {XlatFun, XlatArgs} = prepare_translation_fun(Conf, Schema,
                                                                  Mapping, Xlat),
                    io:format("Running translation for ~s", [Mapping]),
                ...
                _ ->
                    io:format("~p in Translations to drop...", [Mapping]),
                    {Acc, Errors}
            end
        end.
...
add_default(Conf, Prefixes, MappingRecord, Acc) ->
    ...
    case {IsStrictMatch, IsFuzzyMatch} of
        ...
        _ ->
            ...
            io:format("Both fuzzy and strict match! should not happen"),
        ...
        fun({Variable, Value}, {Acc, ErrorAcc}) ->
            ...
            case find_mapping(Variable, Mappings) of
                {error, _} ->
                    ...
                    io:format("You've tried to set ~s, but there is no setting with that name.", [VarName]),
                    io:format("  Did you mean one of these?"),
                    ...
                    _ = [ io:format("    ~s", [T]) || {_, T} <- lists:sublist(Sorted, 3) ],
        ...
        [ begin
            ...
            case {Value, Validator(Value)} of
                ...
                _ ->
                    ...
                    io:format(cuttlefish_error:xlate(Error)),
                    {error, Error}
            end
        end || V <- Vs]
